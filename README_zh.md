...
组件系统、二进制场景保存/加载、OpenGL + GPU Instancing 渲染、可选 Vulkan
RHI、Mono/C# 脚本、2D/3D 物理、音频、材质、Shader，以及基于 ImGui 的编辑器。
...
- Vulkan SDK：检测到 `VULKAN_SDK` 时启用 Vulkan 渲染后端。
...
测试覆盖文件/场景、C# 脚本解析和编译、模拟逻辑，以及 OpenGL 渲染冒烟测试。
